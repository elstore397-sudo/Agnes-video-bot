...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(
        "👋 Kirim foto + deskripsi untuk buat video AI.\n\n"
        "📌 Cara:\n"
        "1. Upload foto\n"
        "2. Kirim deskripsi\n"
...
async def handle_photo(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        await update.message.reply_text(
            "✅ Foto diterima. Kirim deskripsi video:"
        )
    except Exception as e:
...
async def handle_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    keyboard = [
        ...
        [InlineKeyboardButton("🚀 GENERATE!", callback_data="generate_now")]
    ]
    ...
    await update.message.reply_text(
        ...
        "Pilih opsi di bawah, lalu GENERATE!",
...
async def generate_video_process(query: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    async def update_timer():
        nonlocal dot_count, timer_running
        while timer_running:
            dot_count = (dot_count + 1) % 4
            dot_text = "." * dot_count + " " * (3 - dot_count)
            ...
            try:
                ...
                logger.warning(f"Gagal update timer: {e}")
